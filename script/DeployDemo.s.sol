...
/// @dev Defaults are tuned for Base Sepolia. Override via env vars:
...
///        PRICE_USDC    (uint)     price in 6-decimal USDC per whole RWA token
///                                 (e.g. 120_000_000 = 120 USDC / share)
...
contract DeployDemo is Script {
    ...
    function run() public returns (MockRWA token, MockSwap swap) {
        ...
        // 120 USDC per share. Roughly NVIDIA's price at time of writing — owner
        // can `setPrice` later for each demo without redeploying.
        uint256 priceUsdc = vm.envOr("PRICE_USDC", uint256(120 * 1e6));
        ...
        swap = new MockSwap(IERC20(usdc), token, priceUsdc, deployer);
        ...
        console2.log("  price (1e6):   ", priceUsdc);
